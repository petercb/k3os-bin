...
tables with methods like `Expand(sectors)`. However:
- It adds 17 transitive deps (similar count to go-blockdevice/v2)
- It does NOT provide `blkid`-style filesystem probing (label/UUID/type)
- We cannot drop `go-blockdevice/v2` because devpopulate and ProbeFS rely on
  its blkid package
- Adding it would mean maintaining TWO disk libraries for partition ops

If the project ever needs more advanced disk manipulation (filesystem creation,
partition table creation from scratch), go-diskfs would be the consolidation
path. But for just "grow a partition", rekby/mbr (zero deps) is sufficient.

## Recommended Approach: Option B (Pure Go with rekby/mbr)

Chosen approach: use `github.com/rekby/mbr` (MIT, zero transitive deps) for
pure Go MBR manipulation. The library provides Read/Write/GetPartition/SetLBALen
which is everything needed for partition growing.

Implementation:
1. Open device, read MBR with `mbr.Read()`
2. Get partition entry with `GetPartition(num)`
3. Get disk size via `BLKGETSIZE64` ioctl (or file stat for images)
4. Calculate new length: `totalSectors - partStart`
5. `SetLBALen(newLength)`
6. Seek to 0, `Write()` the modified MBR back
7. Sync
