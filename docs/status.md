...
- Replace `doLoopback()` with u-root `libinit.NetInit()` for netlink-based loopback initialization (removes /sbin/ip dependency; kernel 6.8+ auto-configures 127.0.0.1/8 when lo is brought up)
