...
- [ ] 1. Create feature branch and commit spec files
  - [-] 1.1 Create feature branch from master and commit spec
...
- [ ] 2. Remove standalone packages
  - [~] 2.1 Delete `internal/module/` and `internal/sysctl/` directories
...
- [ ] 3. Add integration tests for osimpl adapters
  - [~] 3.1 Create `internal/iface/osimpl/module_test.go`
  ...
  - [~] 3.2 Create `internal/iface/osimpl/sysctl_test.go`
    - Add `//go:build linux` build tag
    - Use `package osimpl_test` (external test package)
  ...
  - [~] 3.3 Run tests via Docker and verify coverage
...
- [ ] 5. Update documentation and finalize
  - [~] 5.1 Update `docs/status.md`
