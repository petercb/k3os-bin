...
- [ ] 1. Create feature branch and commit spec
  - [-] 1.1 Create branch `feature/task-009-pkg-errors-migration` from master and commit the spec
...
- [ ] 2. TDD: Write tests for `internal/util` package
  - [~] 2.1 Create `internal/util/prompt_test.go` with error path tests
  ...
  - [~] 2.2 Write property test for MaskPassword I/O error propagation
  ...
  - [~] 2.3 Run tests via Docker to confirm they pass (or fail only on pre-migration code as expected)
...
- [ ] 3. Migrate `internal/util` and verify
  - [~] 3.1 Apply migration transforms to `internal/util/prompt.go`
...
- [~] 4. Checkpoint - Verify `internal/util` migration
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. TDD: Write tests for `internal/enterchroot` package
  - [~] 5.1 Extract `procFilesystemsPath` variable and create `internal/enterchroot/enter_test.go`
  ...
  - [~] 5.2 Write property test for inProcFS filesystem detection
  ...
  - [~] 5.3 Run tests via Docker to confirm they pass
...
- [ ] 6. Migrate `internal/enterchroot` and verify
  - [~] 6.1 Apply migration transforms to `internal/enterchroot/enter.go`
  ...
  - [~] 6.2 Apply migration transforms to `internal/enterchroot/ensureloop.go`
  ...
  - [~] 6.3 Run tests and lint for `internal/enterchroot`
    - Run: `docker run --rm -v "$(pwd)":/app -w /app golang:1.21.9-bookworm go test -v -race ./internal/enterchroot/...`
    - Run: `golangci-lint run --new-from-rev=master ./internal/enterchroot/...`
    - All tests must pass, zero lint findings
    - _Requirements: 6.2, 6.3_
  - [~] 6.4 Commit `internal/enterchroot` migration
...
- [ ] 7. Remove `pkg/errors` from `go.mod` and verify
  - [~] 7.1 Run `go mod tidy` and verify dependency removal
  ...
  - [~] 7.3 Run lint on all changes
    - Run: `golangci-lint run --new-from-rev=master`
    - Zero new findings
    - _Requirements: 6.3_
  - [~] 7.4 Commit dependency removal
    - Stage `go.mod` and `go.sum`
    - Commit with message `chore: remove pkg/errors dependency`
    - _Requirements: 7.3, 7.4_

- [~] 8. Checkpoint - Full verification
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Update documentation
  - [~] 9.1 Update `docs/status.md`, `docs/log.md`, and `tasks/tasks.md`
